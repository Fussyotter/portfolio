$(() => {
	...
	const makeHobbits = () => {
		ulHobbit = document.createElement('ul');
		...
		document.querySelector('#The-Shire').appendChild(ulHobbit);
	};
	...
	const keepItSecretKeepItSafe = () => {
		...
		oneRing.id = 'the-ring';
		document.querySelectorAll('li.hobbit')[0].appendChild(oneRing);
		console.log(oneRing);
	};
	// ============
	// Chapter 4
	...
	const makeBaddies = () => {
		ulBaddies = document.createElement('ul');

		for (let i = 0; i < baddies.length; i++) {
			const li = document.createElement('li');
			li.classList.add('baddies');
			li.textContent = baddies[i];
			ulBaddies.appendChild(li);
		}
		document.querySelector('#Mordor').appendChild(ulBaddies);
	};
	...
	const makeBuddies = () => {
		...
		const asideBuds = document.createElement('aside');
		document.querySelector('#middle-earth').appendChild(asideBuds);
		document.querySelector('aside').appendChild(ulBuddies);
	};
	...
	const leaveTheShire = () => {
		document.querySelector('#Rivendell').append(ulHobbit);
	};
	...
	const beautifulStranger = () => {
		const listItems = document.querySelectorAll('.buddy');
		console.log(listItems);
		listItems[3].textContent = 'Aragorn';
	};
	...
	const forgeTheFellowShip = () => {
		...
		document
			...
			.appendChild(divFellowship)
			.appendChild(h1Fellowship);
	...
	const theBalrog = () => {
		...
		nameChange.style.border = 'grey';
		nameChange.style.maxWidth = '100px';
	};
	...
	const hornOfGondor = () => {
		...
		document.getElementsByClassName('buddy')[4].style.textDecoration =
			'line-through';
		const removed = document.getElementsByClassName('baddies')[2];
		removed.remove();
	};
	...
	const weWantsIt = () => {
		...
		const theRing =
			document.getElementById('mount-doom').childNodes[0].childNodes[1];
	...
	const thereAndBackAgain = () => {
		...
		const baddies = document.querySelectorAll('.baddies');
		for (let i = 0; i < baddies.length; i++) {
			baddies[i].remove();
		}

		// for (let i = 0; i < ulBaddies.length; i++) {
		// 	ulBaddies[i].remove();
		// }
		// mountDoom.appendChild(frodoMovedToMordor);
		// mountDoom.appendChild(samMovedToMordor);
		...
		document
			...
			.appendChild(ulHobbit)
			.appendChild(hobbits);
		document.querySelector('#The-Shire').appendChild(frodoMovedToMordor);
	};
	// =====================================
